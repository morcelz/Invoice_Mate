...
export default function Clients() {
  ...
  const handleEditClient = async () => {
    try {
      ...
      const token = await AsyncStorage.getItem('token');
      const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/clients/${editingClient.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      const newClientData = await response.json();
      setClients([...clients.filter(client => client.id !== editingClient.id), newClientData.client]);
    } catch (error) {
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const token = await AsyncStorage.getItem('token');
              const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/clients/${id}`, {
                method: 'DELETE',
              ...
              Alert.alert('Error', 'Something went wrong while deleting the client.');
            }
          },
        },
      ],
  ...
  const toggleAccordion = (id: string) => {
    setClients(
      clients.map((client) =>
        client.id === id ? { ...client, isExpanded: !client.isExpanded } : client
      )
    );
  };
  useEffect(() => {
  ...
  const renderClientItem = ({ item }: { item: typeof clients[0] }) => (
    <View style={styles.clientItem}>
      <TouchableOpacity onPress={() => toggleAccordion(item.id)} style={styles.clientHeader}>
      ...
      {item.isExpanded && (
        <View style={styles.clientDetails}>
          ...
          <View style={styles.clientActions}>
            <TouchableOpacity onPress={() => openEditClientModal(item)} style={styles.actionButton}>
              <Ionicons name="pencil" size={20} color="#007BFF" />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => handleDeleteClient(item.id)} style={styles.actionButton}>
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <TextInput
              ...
              onChangeText={(text) => setNewClient({ ...newClient, fisical_code: text })}
            />
            <TextInput
              placeholder="address *"
              style={styles.underlinedInput}
            ...
            <TextInput
              ...
              onChangeText={(text) => setNewClient({ ...newClient, zip_code: text })}
            />
            <Picker
              selectedValue={newClient.country}
              onValueChange={(itemValue) => setNewClient({ ...newClient, country: itemValue })}
              style={styles.picker}
            >
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity onPress={closeAddClientModal} style={styles.cancelButton}>
                <Text style={styles.buttonText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleAddClient} style={styles.addClientButton}>
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <TextInput
              ...
              onChangeText={(text) => setEditingClient({ ...editingClient, company_name: text })}
            />
            <TextInput
              placeholder="Fiscal Code"
              style={styles.underlinedInput}
              value={editingClient.fisical_code}
              onChangeText={(text) => setEditingClient({ ...editingClient, fisical_code: text })}
            />
            <TextInput
              placeholder="Address *"
              style={styles.underlinedInput}
            ...
            <TextInput
              ...
              onChangeText={(text) => setEditingClient({ ...editingClient, zip_code: text })}
            />
            <Picker
              selectedValue={editingClient.country}
              onValueChange={(itemValue) => setEditingClient({ ...editingClient, country: itemValue })}
              style={styles.picker}
            >
              {countries.map((country) => (
                <Picker.Item key={country.value} label={country.label} value={country.value} />
            ...
            />
            <View style={styles.modalButtons}>
              <TouchableOpacity onPress={closeEditClientModal} style={styles.cancelButton}>
                <Text style={styles.buttonText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleEditClient} style={styles.addClientButton}>
